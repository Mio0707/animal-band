...
function learningCard(item, rhythmAsset = null, teachingAssets = null) {
  const description = item.description || item.rule || (item.notation ? `节奏示例：${item.notation}` : "本学段需要体验和掌握的音乐内容。");
  return `<article class="knowledge-card"><span>学习内容</span><h3>${escapeHtml(item.name)}</h3><p>${escapeHtml(description)}</p>${item.notation ? `<strong>${escapeHtml(item.notation)}</strong>` : ""}${rhythmPreview(rhythmAsset, teachingAssets)}</article>`;
}
...
export function renderKnowledgeBase(data, params, gradeBand = "1-2") {
  ...
  const teachingAssets = data.rhythmConfig?.teachingAssets;
  const rhythmAssets = new Map((teachingAssets?.rhythmTeachingAssets ?? []).map((asset) => [asset.materialId, asset]));
  return `<main class="knowledge-page"><a class="back-link" href="#/songs?grade=1-2">← 返回歌曲库</a><header class="knowledge-heading"><p class="eyebrow">第一学段 · 1–2年级</p><h1>音乐学习知识库</h1><p>查看这个学段需要体验、理解和练习的具体内容。</p></header><nav class="knowledge-tabs">${AREAS.map(([id, label]) => `<a class="${area === id ? "active" : ""}" href="#/knowledge?grade=1-2&area=${id}">${label}</a>`).join("")}</nav>${materials.length ? `<section><div class="knowledge-section-title"><h2>练习材料</h2><span>${materials.length}项</span></div><div class="knowledge-grid">${materials.map((item) => learningCard(item, rhythmAssets.get(item.id), teachingAssets)).join("")}</div></section>` : ""}<section><div class="knowledge-section-title"><h2>学习目标</h2><span>${targets.length}项</span></div><div class="knowledge-grid">${targets.map((item) => learningCard(item)).join("")}</div></section></main>`;
}
