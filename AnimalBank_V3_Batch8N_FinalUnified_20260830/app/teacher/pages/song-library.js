...
function songCard(data, song) {
  ...
  const classroomHref = preparation ? `/app/classroom/?preparation=${encodeURIComponent(preparation.preparationId)}&mode=live` : "";
  const offlineHref = preparation ? `/api/preparations/${encodeURIComponent(preparation.preparationId)}/offline-package` : "";
  return `<article class="teacher-song-card" data-song-title="${escapeHtml(song.title.toLowerCase())}"><div class="song-art"><span>♪</span></div><div class="song-card-copy"><small>${teacherStateLabel(state)}</small><h2>《${escapeHtml(song.title)}》</h2><p>${state === "NOT_PREPARED" ? "选择这首歌开始设计课堂" : state === "READY" ? "课堂内容已经准备完成" : `上次保存：${escapeHtml(updated || "刚刚")}`}</p></div><div class="song-card-actions"><button class="button primary" data-open-preparation="${escapeHtml(song.songId)}">${actionLabel(state)}</button>${state === "READY" ? `<a class="button secondary" data-start-class href="${escapeHtml(classroomHref)}">开始上课</a><a class="button offline-download" data-download-offline href="${escapeHtml(offlineHref)}" download>下载离线课</a>` : ""}</div></article>`;
}
