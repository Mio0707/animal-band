...
export async function isUsableVerifiedScore(score) {
  ...
  return validation.valid
    ...
    && Number.isInteger(Number(score.teachingConfig?.singingMeasuresPerUnit));
}

export async function assertUsableVerifiedScore(score) {
  if (!(await isUsableVerifiedScore(score))) {
    throw new Error("VerifiedScore 尚不可用于下游：必须通过 Schema 校验、状态为 verified，并记录人工审核人。");
  }
  return score;
}

export async function saveVerifiedScore(score, filePath) {
  await assertUsableVerifiedScore(score);
