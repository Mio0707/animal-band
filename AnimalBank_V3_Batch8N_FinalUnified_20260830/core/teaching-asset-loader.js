...
export async function validateTeachingAssetReferences(library = undefined, curriculum = undefined) {
  ...
  const errors = [];
  const rhythmStandards = new Map((curriculumSource.modules.rhythm.material_catalog ?? []).map((material) => [material.id, material]));
  const rhythmAssets = new Map((source.rhythmTeachingAssets ?? []).filter((asset) => /^PAT-\d{2}$/.test(asset.materialId ?? "")).map((asset) => [asset.materialId, asset]));

  for (const [materialId, standard] of rhythmStandards) {
    const asset = rhythmAssets.get(materialId);
    if (!asset) {
      errors.push({ materialId, path: "$.rhythmTeachingAssets", message: `课程标准节奏型 ${materialId} 缺少对应的 Teaching Asset。` });
      continue;
    }
    for (const field of ["notation", "chant", "durations"]) {
      if (JSON.stringify(asset[field]) !== JSON.stringify(standard[field])) {
        errors.push({ assetId: asset.assetId, field, path: `$.rhythmTeachingAssets.${materialId}.${field}`, message: `${materialId}.${field} 必须与课程标准完全一致。` });
      }
    }
  }
  for (const materialId of rhythmAssets.keys()) {
    if (!rhythmStandards.has(materialId)) errors.push({ materialId, path: "$.rhythmTeachingAssets", message: `Teaching Asset 包含课程标准之外的节奏型 ${materialId}。` });
  }
