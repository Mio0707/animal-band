...
QWEN_API_KEY_REQUIRED_MESSAGE = "当前功能需要 Qwen API Key，请联系开发者使用。"
...
def qwen_configuration_status() -> dict:
    """Expose configuration availability without ever returning the secret."""
    load_dotenv()
    configured = bool(str(os.environ.get("DASHSCOPE_API_KEY") or "").strip())
    return {
        "configured": configured,
        "message": None if configured else QWEN_API_KEY_REQUIRED_MESSAGE,
        "envFile": ".env",
    }
...
规则：degree 取 1–7，休止符 degree=0 且 rest=true；octave 中音=0、下方一点=-1、上方一点=1；beat 和 duration 以四分音符为 1 拍；小节内 beat 从 0 开始。lyric 为该音符对应的单个汉字，无歌词或无法确认时为 null，禁止猜测；一字多音时，第一个音符写 lyric=该汉字，后续音符必须 lyric=null 且 lyricContinuation=true；这表示同一个字延续到新的音高，不是再次唱这个字。lyricsText 保存按演唱顺序识别到的完整歌词。不确定时降低 confidence 并写入 warnings。"""
...
规则：休止符 lyric=null；普通音符 lyric 为对应的单个汉字；标点符号绝不能占用音符；无歌词或看不清时 lyric=null，禁止猜测；一字多音或延长音时，第一个音符写该汉字，后续音符必须 lyric=null 且 lyricContinuation=true；不确定项降低 confidence 并写入 warnings。
...
def merge_recognized_lyrics(score: dict, raw: dict) -> dict:
    ...
    for note in flat_notes:
        ...
        if lyric and re.fullmatch(r"[\s，。！？、；：,.!?;:（）()“”\"'—-]+", lyric):
            lyric = None
            continuation = bool(previous_lyric and previous_syllable_id)
        if note.get("rest"):
            lyric = None
            continuation = False
        if continuation and previous_lyric and previous_syllable_id:
            lyric = None
