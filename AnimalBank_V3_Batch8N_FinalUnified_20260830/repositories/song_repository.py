...
class SongRepository:
    ...
    def save_original_audio(self, song_id: str, audio: tuple[str, bytes]) -> dict:
        ...
        audio_name, audio_bytes = audio
        source_dir = self._song_dir(song_id) / "source"
    ...
    def get_score(self, song_id: str) -> dict | None:
        ...
        if not song:
            raise KeyError(song_id)
        score_state = song["score"]
        use_verified = score_state.get("verificationStatus") == "verified" or song["processingStatus"] in {"SCORE_VERIFIED", "PROFILE_READY"}
        relative = score_state.get("verifiedPath") if use_verified else score_state.get("draftPath")
        if not relative:
            return None
    ...
    def artifact_path(self, song_id: str, name: str) -> Path:
        song_id = require_song_id(song_id)
        allowed = {"material-match.json", "learning-profile.json", "melody-trace-plan.json", "measure-alignment.json", "ensemble-plan.json", "listening-body-plan.json", "sticker-stems.json", "sticker-arrangement-plan.json"}
    ...
    def delete_artifact(self, song_id: str, name: str) -> None:
        self.artifact_path(song_id, name).unlink(missing_ok=True)
