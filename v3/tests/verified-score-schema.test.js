...
import {
  assertPublishableVerifiedScore,
...
test("VerifiedScore fixture 可以成功读取和发布", async () => {
  const score = await loadVerifiedScore();

  assert.equal(score.verificationStatus, "verified");
  assert.equal(await isPublishableVerifiedScore(score), true);
});

test("draft 和 reviewed Score 都不能成为后续可发布数据", async () => {
  ...
  for (const status of ["draft", "reviewed"]) {
    ...
    assert.equal(await isPublishableVerifiedScore(score), false);
    await assert.rejects(() => assertPublishableVerifiedScore(score), /不可发布/);
  }
});
